...
class UsuarioViewModel : ViewModel() {
    private var _usuarios = mutableStateOf<List<Usuario>>(emptyList())
    var usuarios: State<List<Usuario>> = _usuarios
    ...
    fun listarUsuarios() {
        coroutineScope.launch {
            try {
                _usuarios.value = RetrofitClient.instance.listarUsuarios()
    ...
    fun buscarUsuario(id: Int) {
        coroutineScope.launch {
            try {
                _usuarioDetalhe.value = RetrofitClient.instance.buscarUsuario(id)
    ...
    fun criarUsuario(usuario: Usuario) {
        coroutineScope.launch {
            try {
                val novoUsuario = RetrofitClient.instance.criarUsuario(usuario)
    ...
    fun atualizarUsuario(id: Int, usuarioAtualizado: Usuario) {
        coroutineScope.launch {
            try {
                val response = RetrofitClient.instance.atualizarUsuario(id, usuarioAtualizado)
    ...
    fun deletarUsuario(id: Int) {
        coroutineScope.launch {
            try {
                val response = RetrofitClient.instance.deletarUsuario(id)
                if (response.isSuccessful) {
